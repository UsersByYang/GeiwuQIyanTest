Shader "Ciconia Studio/CS_Standard/Builtin/Standard/Transparent"
{
	Properties
	{
	...
	SubShader
	{
		...
		UNITY_INSTANCING_BUFFER_START(CiconiaStudioCS_StandardBuiltinStandardTransparent)
			UNITY_DEFINE_INSTANCED_PROP(float, _TriplanarFalloff)
#define _TriplanarFalloff_arr CiconiaStudioCS_StandardBuiltinStandardTransparent
		UNITY_INSTANCING_BUFFER_END(CiconiaStudioCS_StandardBuiltinStandardTransparent)
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 break26_g495 = uv_BumpMap;
			...
			float3 temp_output_356_0 = UnpackScaleNormal( tex2D( _BumpMap, ( ( appendResult14_g495 + appendResult13_g495 ) + Parallaxe382 ) ), _BumpScale );
			...
			float4 clampResult222 = clamp( ( CalculateContrast(( _ContrastDetailMap + 1.0 ),(( _InvertMask )?( ( 1.0 - (( _EnableTriplanarProjection )?( triplanar218 ):( tex2DNode225 )) ) ):( (( _EnableTriplanarProjection )?( triplanar218 ):( tex2DNode225 )) ))) + -_SpreadDetailMap ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
			...
			float2 break26_g493 = uv_MainTex;
			float2 appendResult14_g493 = (float2(( break26_g493.x * GlobalTilingX11 ) , ( break26_g493.y * GlobalTilingY8 )));
			float2 appendResult13_g493 = (float2(( break26_g493.x + GlobalOffsetX10 ) , ( break26_g493.y + GlobalOffsetY9 )));
			float4 tex2DNode7_g492 = tex2D( _MainTex, ( ( appendResult14_g493 + appendResult13_g493 ) + Parallaxe382 ) );
			float clampResult27_g492 = clamp( _Saturation , -1.0 , 100.0 );
			float3 desaturateInitialColor29_g492 = ( _Color * tex2DNode7_g492 ).rgb;
			float desaturateDot29_g492 = dot( desaturateInitialColor29_g492, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar29_g492 = lerp( desaturateInitialColor29_g492, desaturateDot29_g492.xxx, -clampResult27_g492 );
			float4 temp_output_393_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g492 , 0.0 ));
			...
			float4 lerpResult343 = lerp( temp_output_393_0 , AlbedoDetail153 , Mask158);
			float4 blendOpSrc15_g519 = temp_output_393_0;
			float4 blendOpDest15_g519 = lerpResult343;
			float4 lerpBlendMode15_g519 = lerp(blendOpDest15_g519,(( blendOpDest15_g519 > 0.5 ) ? ( 1.0 - 2.0 * ( 1.0 - blendOpDest15_g519 ) * ( 1.0 - blendOpSrc15_g519 ) ) : ( 2.0 * blendOpDest15_g519 * blendOpSrc15_g519 ) ),Mask158.x);
			float4 Albedo26 = (( _VisualizeMask )?( Mask158 ):( (( _BlendmodeOverlay )?( ( saturate( lerpBlendMode15_g519 )) ):( lerpResult343 )) ));
			o.Albedo = Albedo26.rgb;
			float2 uv_EmissionMap = i.uv_texcoord * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
			float2 break26_g518 = uv_EmissionMap;
			...
			float4 temp_output_359_0 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g518 + appendResult13_g518 ) + Parallaxe382 ) ) * _EmissionIntensity );
			...
			float2 break26_g521 = uv_MetallicGlossMap;
			float2 appendResult14_g521 = (float2(( break26_g521.x * GlobalTilingX11 ) , ( break26_g521.y * GlobalTilingY8 )));
			float2 appendResult13_g521 = (float2(( break26_g521.x + GlobalOffsetX10 ) , ( break26_g521.y + GlobalOffsetY9 )));
			float4 tex2DNode3_g520 = tex2D( _MetallicGlossMap, ( ( appendResult14_g521 + appendResult13_g521 ) + Parallaxe382 ) );
			float2 uv_DetailMetallicGlossMap = i.uv_texcoord * _DetailMetallicGlossMap_ST.xy + _DetailMetallicGlossMap_ST.zw;
			float4 tex2DNode3_g496 = tex2D( _DetailMetallicGlossMap, uv_DetailMetallicGlossMap );
			float DetailMetallic176 = ( tex2DNode3_g496.r * _DetailMetallic );
			float lerpResult179 = lerp( ( tex2DNode3_g520.r * _Metallic ) , DetailMetallic176 , Mask158.r);
			float Metallic41 = lerpResult179;
			o.Metallic = Metallic41;
			float BaseColorAlpha46 = (( _InvertABaseColor )?( ( 1.0 - tex2DNode7_g492.a ) ):( tex2DNode7_g492.a ));
			#if defined(_SOURCE_METALLICALPHA)
				float staticSwitch23_g520 = ( tex2DNode3_g520.a * _Glossiness );
			#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch23_g520 = ( _Glossiness * BaseColorAlpha46 );
			#else
				float staticSwitch23_g520 = ( tex2DNode3_g520.a * _Glossiness );
			#endif
			float DetailBaseColorAlpha170 = tex2DNode7_g468.a;
			#if defined(_DETAILSOURCE_METALLICALPHA)
				float staticSwitch23_g496 = ( tex2DNode3_g496.a * _DetailGlossiness );
			#elif defined(_DETAILSOURCE_BASECOLORALPHA)
				float staticSwitch23_g496 = ( _DetailGlossiness * DetailBaseColorAlpha170 );
			#else
				float staticSwitch23_g496 = ( tex2DNode3_g496.a * _DetailGlossiness );
			#endif
			float DetailSmoothness175 = staticSwitch23_g496;
			float lerpResult182 = lerp( staticSwitch23_g520 , DetailSmoothness175 , Mask158.r);
			float Smoothness40 = lerpResult182;
			o.Smoothness = Smoothness40;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float2 break26_g516 = uv_OcclusionMap;
			float2 appendResult14_g516 = (float2(( break26_g516.x * GlobalTilingX11 ) , ( break26_g516.y * GlobalTilingY8 )));
			float2 appendResult13_g516 = (float2(( break26_g516.x + GlobalOffsetX10 ) , ( break26_g516.y + GlobalOffsetY9 )));
			float blendOpSrc2_g515 = tex2D( _OcclusionMap, ( ( appendResult14_g516 + appendResult13_g516 ) + Parallaxe382 ) ).r;
			float blendOpDest2_g515 = ( 1.0 - _AoIntensity );
			float temp_output_358_0 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g515 ) * ( 1.0 - blendOpDest2_g515 ) ) ));
			...
			float4 tex2DNode8_g522 = tex2D( _TransparentMask, uv_TransparentMask );
			float4 temp_cast_26 = (BaseColorAlpha46).xxxx;
			float4 temp_cast_27 = (BaseColorAlpha46).xxxx;
			float4 clampResult24_g522 = clamp( ( CalculateContrast(( _ContrastTransparentMap + 1.0 ),(( _InvertTransparent )?( ( 1.0 - (( _UseBaseColorAlpha )?( temp_cast_27 ):( tex2DNode8_g522 )) ) ):( (( _UseBaseColorAlpha )?( temp_cast_26 ):( tex2DNode8_g522 )) ))) + -_SpreadTransparentMap ) , float4( 0,0,0,0 ) , float4( 1,0,0,0 ) );
			float4 Transparent368 = ( clampResult24_g522 * ( 1.0 - _IntensityTransparentMap ) );
			o.Alpha = Transparent368.r;
		}
