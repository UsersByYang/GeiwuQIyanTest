Shader "Ciconia Studio/Standard (Double Sided) V2 Legacy/MetalRough/Advanced (Both Sides)/Cutout"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 switchResult209 = (((i.ASEVFace>0)?(lerpResult5_g91):(lerpResult5_g90)));
			float3 Normalmap19 = switchResult209;
			o.Normal = Normalmap19;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g114 = tex2D( _MainTex, ( uv_MainTex * TilingLayer1148 ) );
			...
			float clampResult15_g114 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g114 = (temp_cast_3 + (pow( tex2DNode7_g114 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g114 = dot( desaturateInitialColor16_g114, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g114 = lerp( desaturateInitialColor16_g114, desaturateDot16_g114.xxx, clampResult15_g114 );
			float2 uv_DetailAlbedoMap = i.uv_texcoord * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
			float4 tex2DNode7_g113 = tex2D( _DetailAlbedoMap, ( uv_DetailAlbedoMap * TilingLayer2147 ) );
			...
			float clampResult15_g113 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g113 = (temp_cast_10 + (pow( tex2DNode7_g113 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g113 = dot( desaturateInitialColor16_g113, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g113 = lerp( desaturateInitialColor16_g113, desaturateDot16_g113.xxx, clampResult15_g113 );
			float4 switchResult206 = (((i.ASEVFace>0)?(( _Color * float4( desaturateVar16_g114 , 0.0 ) )):(( _Color2 * float4( desaturateVar16_g113 , 0.0 ) ))));
			float4 Albedomap34 = switchResult206;
			o.Albedo = Albedomap34.rgb;
			float2 uv_EmissionMap = i.uv_texcoord * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
			float2 uv_EmissionMap2 = i.uv_texcoord * _EmissionMap2_ST.xy + _EmissionMap2_ST.zw;
			float4 switchResult211 = (((i.ASEVFace>0)?(( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * TilingLayer1148 ) ) * _EmissiveIntensity1 )):(( _EmissionColor2 * tex2D( _EmissionMap2, ( uv_EmissionMap2 * TilingLayer2147 ) ) * _EmissiveIntensity2 ))));
			...
			float4 lerpResult214 = lerp( temp_cast_18 , temp_output_132_0 , _DoubleSidedBlend);
			float4 switchResult213 = (((i.ASEVFace>0)?(temp_output_132_0):(lerpResult214)));
			float temp_output_8_0_g169 = Albedomap34.r;
			float4 temp_output_5_0_g169 = ( temp_output_8_0_g169 * UNITY_LIGHTMODEL_AMBIENT );
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float fresnelNdotV11_g170 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode11_g170 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV11_g170, 5.0 ) );
			float4 Emissivemap31 = ( switchResult211 + ( lerp(temp_cast_15,switchResult213,_EnableReflection) + ( temp_output_5_0_g169 * _AmbientLight1 ) + ( ( ( 0.95 * fresnelNode11_g170 ) + 0.05 ) * _FresnelStrength1 ) ) );
			o.Emission = Emissivemap31.rgb;
			float2 uv_MetallicGlossMap = i.uv_texcoord * _MetallicGlossMap_ST.xy + _MetallicGlossMap_ST.zw;
			float4 tex2DNode3_g172 = tex2D( _MetallicGlossMap, ( uv_MetallicGlossMap * TilingLayer1148 ) );
			float2 uv_MetallicGlossMap2 = i.uv_texcoord * _MetallicGlossMap2_ST.xy + _MetallicGlossMap2_ST.zw;
			float4 tex2DNode3_g173 = tex2D( _MetallicGlossMap2, ( uv_MetallicGlossMap2 * TilingLayer1148 ) );
			float switchResult208 = (((i.ASEVFace>0)?(( tex2DNode3_g172.r * _Metallic )):(( tex2DNode3_g173.r * _Metallic2 ))));
			float Metallic37 = switchResult208;
			o.Metallic = Metallic37;
			float switchResult207 = (((i.ASEVFace>0)?(( tex2DNode3_g172.a * _Glossiness )):(( tex2DNode3_g173.a * _Glossiness2 ))));
			float Roughness38 = switchResult207;
			o.Smoothness = Roughness38;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g174 = tex2D( _OcclusionMap, ( uv_OcclusionMap * TilingLayer1148 ) ).r;
			float blendOpDest2_g174 = ( 1.0 - _AoIntensity1 );
			float2 uv_OcclusionMap2 = i.uv_texcoord * _OcclusionMap2_ST.xy + _OcclusionMap2_ST.zw;
			float blendOpSrc2_g171 = tex2D( _OcclusionMap2, ( uv_OcclusionMap2 * TilingLayer2147 ) ).r;
			float blendOpDest2_g171 = ( 1.0 - _AoIntensity2 );
			float switchResult210 = (((i.ASEVFace>0)?(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g174 ) * ( 1.0 - blendOpDest2_g174 ) ) ))):(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g171 ) * ( 1.0 - blendOpDest2_g171 ) ) )))));
			float Aomap41 = switchResult210;
			o.Occlusion = Aomap41;
			o.Alpha = 1;
			float AAlbedo1171 = tex2DNode7_g114.a;
			float temp_output_1_0_g175 = AAlbedo1171;
			float AAlbedo2172 = tex2DNode7_g113.a;
			float temp_output_2_0_g175 = AAlbedo2172;
			float switchResult40_g175 = (((i.ASEVFace>0)?(lerp(lerp(temp_output_1_0_g175,( 1.0 - temp_output_1_0_g175 ),_InvertAlbedoAlpha),1.0,_DisableAlbedoAlphaChannel)):(lerp(lerp(temp_output_2_0_g175,( 1.0 - temp_output_2_0_g175 ),_InvertAlbedo2Alpha),1.0,_DisableAlbedo2AlphaChannel))));
			float Cutout178 = switchResult40_g175;
