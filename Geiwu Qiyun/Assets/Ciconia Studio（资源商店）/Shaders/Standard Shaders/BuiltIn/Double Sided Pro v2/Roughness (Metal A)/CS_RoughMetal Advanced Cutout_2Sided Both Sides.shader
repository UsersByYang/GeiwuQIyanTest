Shader "Ciconia Studio/Standard (Double Sided) V2 Legacy/Roughness Setup/Advanced (Both Sides)/Cutout"
{
	Properties
	{
		...
		[Space(35)]_SpecGlossMap("Roughness(MetallicA)", 2D) = "white" {}
        ...
        [Space(15)][Toggle]_EnableReflection("Enable Reflection", Float) = 0
		...
		[Space(35)]_SpecGlossMap2("Roughness(MetallicA)", 2D) = "white" {}
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult5_g95 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * TilingLayer1148 ) ) ) , _BumpScale);
			float2 uv_DetailNormalMap = i.uv_texcoord * _DetailNormalMap_ST.xy + _DetailNormalMap_ST.zw;
			float TilingLayer2147 = _GeneralTilingLayer2;
			float3 lerpResult5_g94 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _DetailNormalMap, ( uv_DetailNormalMap * TilingLayer2147 ) ) ) , _DetailNormalMapScale);
			float3 switchResult213 = (((i.ASEVFace>0)?(lerpResult5_g95):(lerpResult5_g94)));
			float3 Normalmap19 = switchResult213;
			o.Normal = Normalmap19;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g116 = tex2D( _MainTex, ( uv_MainTex * TilingLayer1148 ) );
			...
			float clampResult15_g116 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g116 = (temp_cast_3 + (pow( tex2DNode7_g116 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g116 = dot( desaturateInitialColor16_g116, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g116 = lerp( desaturateInitialColor16_g116, desaturateDot16_g116.xxx, clampResult15_g116 );
			float2 uv_DetailAlbedoMap = i.uv_texcoord * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
			float4 tex2DNode7_g117 = tex2D( _DetailAlbedoMap, ( uv_DetailAlbedoMap * TilingLayer2147 ) );
			...
			float clampResult15_g117 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g117 = (temp_cast_10 + (pow( tex2DNode7_g117 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g117 = dot( desaturateInitialColor16_g117, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g117 = lerp( desaturateInitialColor16_g117, desaturateDot16_g117.xxx, clampResult15_g117 );
			float4 switchResult210 = (((i.ASEVFace>0)?(( _Color * float4( desaturateVar16_g116 , 0.0 ) )):(( _Color2 * float4( desaturateVar16_g117 , 0.0 ) ))));
			float4 Albedomap34 = switchResult210;
			o.Albedo = Albedomap34.rgb;
			float2 uv_EmissionMap = i.uv_texcoord * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
			float2 uv_EmissionMap2 = i.uv_texcoord * _EmissionMap2_ST.xy + _EmissionMap2_ST.zw;
			float4 switchResult215 = (((i.ASEVFace>0)?(( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * TilingLayer1148 ) ) * _EmissiveIntensity1 )):(( _EmissionColor2 * tex2D( _EmissionMap2, ( uv_EmissionMap2 * TilingLayer2147 ) ) * _EmissiveIntensity2 ))));
			...
			float4 lerpResult216 = lerp( temp_cast_18 , temp_output_132_0 , _DoubleSidedBlend);
			float4 switchResult217 = (((i.ASEVFace>0)?(temp_output_132_0):(lerpResult216)));
			float temp_output_8_0_g180 = Albedomap34.r;
			float4 temp_output_5_0_g180 = ( temp_output_8_0_g180 * UNITY_LIGHTMODEL_AMBIENT );
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float fresnelNdotV11_g182 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode11_g182 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV11_g182, 5.0 ) );
			float4 Emissivemap31 = ( switchResult215 + ( lerp(temp_cast_15,switchResult217,_EnableReflection) + ( temp_output_5_0_g180 * _AmbientLight1 ) + ( ( ( 0.95 * fresnelNode11_g182 ) + 0.05 ) * _FresnelStrength1 ) ) );
			o.Emission = Emissivemap31.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g188 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * TilingLayer1148 ) );
			float2 uv_SpecGlossMap2 = i.uv_texcoord * _SpecGlossMap2_ST.xy + _SpecGlossMap2_ST.zw;
			float4 tex2DNode3_g190 = tex2D( _SpecGlossMap2, ( uv_SpecGlossMap2 * TilingLayer1148 ) );
			float switchResult212 = (((i.ASEVFace>0)?(( tex2DNode3_g188.a * _Metallic )):(( tex2DNode3_g190.a * _Metallic2 ))));
			float Metallic37 = switchResult212;
			o.Metallic = Metallic37;
			float switchResult211 = (((i.ASEVFace>0)?(( tex2DNode3_g188.r * _Glossiness )):(( tex2DNode3_g190.r * _Glossiness2 ))));
			float Roughness38 = switchResult211;
			o.Smoothness = Roughness38;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g189 = tex2D( _OcclusionMap, ( uv_OcclusionMap * TilingLayer1148 ) ).r;
			float blendOpDest2_g189 = ( 1.0 - _AoIntensity1 );
			float2 uv_OcclusionMap2 = i.uv_texcoord * _OcclusionMap2_ST.xy + _OcclusionMap2_ST.zw;
			float blendOpSrc2_g187 = tex2D( _OcclusionMap2, ( uv_OcclusionMap2 * TilingLayer2147 ) ).r;
			float blendOpDest2_g187 = ( 1.0 - _AoIntensity2 );
			float switchResult214 = (((i.ASEVFace>0)?(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g189 ) * ( 1.0 - blendOpDest2_g189 ) ) ))):(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g187 ) * ( 1.0 - blendOpDest2_g187 ) ) )))));
			float Aomap41 = switchResult214;
			o.Occlusion = Aomap41;
			o.Alpha = 1;
			float AAlbedo1171 = tex2DNode7_g116.a;
			float temp_output_1_0_g191 = AAlbedo1171;
			float AAlbedo2172 = tex2DNode7_g117.a;
			float temp_output_2_0_g191 = AAlbedo2172;
			float switchResult40_g191 = (((i.ASEVFace>0)?(lerp(lerp(temp_output_1_0_g191,( 1.0 - temp_output_1_0_g191 ),_InvertAlbedoAlpha),1.0,_DisableAlbedoAlphaChannel)):(lerp(lerp(temp_output_2_0_g191,( 1.0 - temp_output_2_0_g191 ),_InvertAlbedo2Alpha),1.0,_DisableAlbedo2AlphaChannel))));
			float Cutout178 = switchResult40_g191;
