Shader "Ciconia Studio/Standard (Double Sided) V2 Legacy/Roughness Setup/Fast/Cutout"
{
	Properties
	{
		...
		[Space(35)]_Metallic("Metallic", Range( 0 , 2)) = 0.2
		...
		[Space(15)][Toggle]_InvertAdditionalmap("Invert Additional map", Float) = 0
		_Additionalmap("Additional map", 2D) = "black" {}
	...
	SubShader
	{
		...
		uniform float _InvertAdditionalmap;
		uniform sampler2D _Additionalmap;
		uniform float4 _Additionalmap_ST;
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float4 tex2DNode7_g58 = tex2D( _MainTex, ( uv_MainTex * TilingLayer1105 ) );
			...
			float clampResult15_g58 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g58 = (temp_cast_3 + (pow( tex2DNode7_g58 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g58 = dot( desaturateInitialColor16_g58, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g58 = lerp( desaturateInitialColor16_g58, desaturateDot16_g58.xxx, clampResult15_g58 );
			float4 Albedomap34 = ( _Color * float4( desaturateVar16_g58 , 0.0 ) );
			...
			float temp_output_8_0_g62 = Albedomap34.r;
			float4 temp_output_5_0_g62 = ( temp_output_8_0_g62 * UNITY_LIGHTMODEL_AMBIENT );
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 ase_worldNormal = i.worldNormal;
			float fresnelNdotV11_g63 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode11_g63 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV11_g63, 5.0 ) );
			float4 Emissivemap31 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * TilingLayer1105 ) ) * _EmissiveIntensity1 ) + ( lerp(temp_cast_8,( _CubemapColor * float4( desaturateVar101 , 0.0 ) ),_EnableReflection) + ( temp_output_5_0_g62 * _AmbientLight1 ) + ( ( ( 0.95 * fresnelNode11_g63 ) + 0.05 ) * _FresnelStrength1 ) ) );
			o.Emission = Emissivemap31.rgb;
			float Metallic95 = _Metallic;
			o.Metallic = Metallic95;
			float Roughness94 = _Glossiness;
			o.Smoothness = Roughness94;
			o.Alpha = 1;
			float AAlbedo1117 = tex2DNode7_g58.a;
			float temp_output_1_0_g65 = AAlbedo1117;
			float2 uv_Additionalmap = i.uv_texcoord * _Additionalmap_ST.xy + _Additionalmap_ST.zw;
			float4 tex2DNode28_g65 = tex2D( _Additionalmap, uv_Additionalmap );
			float blendOpSrc26_g65 = lerp(lerp(temp_output_1_0_g65,( 1.0 - temp_output_1_0_g65 ),_InvertAlbedoAlpha),1.0,_DisableAlbedoAlphaChannel);
			float blendOpDest26_g65 = lerp(( 1.0 - tex2DNode28_g65.r ),tex2DNode28_g65.r,_InvertAdditionalmap);
			float clampResult36_g65 = clamp( ( saturate( ( blendOpSrc26_g65 * blendOpDest26_g65 ) )) , 0.0 , 1.0 );
			float Opacity115 = clampResult36_g65;
			clip( Opacity115 - _Cutoff );
		}
