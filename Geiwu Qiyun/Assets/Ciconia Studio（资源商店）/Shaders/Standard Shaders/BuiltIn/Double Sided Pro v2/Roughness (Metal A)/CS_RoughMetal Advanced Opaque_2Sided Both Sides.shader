Shader "Ciconia Studio/Standard (Double Sided) V2 Legacy/Roughness Setup/Advanced (Both Sides)/Opaque"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult5_g93 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * TilingLayer1148 ) ) ) , _BumpScale);
			float2 uv_DetailNormalMap = i.uv_texcoord * _DetailNormalMap_ST.xy + _DetailNormalMap_ST.zw;
			float TilingLayer2147 = _GeneralTilingLayer2;
			float3 lerpResult5_g92 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _DetailNormalMap, ( uv_DetailNormalMap * TilingLayer2147 ) ) ) , _DetailNormalMapScale);
			float3 switchResult178 = (((i.ASEVFace>0)?(lerpResult5_g93):(lerpResult5_g92)));
			float3 Normalmap19 = switchResult178;
			o.Normal = Normalmap19;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g95 = tex2D( _MainTex, ( uv_MainTex * TilingLayer1148 ) );
			...
			float clampResult15_g95 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g95 = (temp_cast_3 + (pow( tex2DNode7_g95 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g95 = dot( desaturateInitialColor16_g95, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g95 = lerp( desaturateInitialColor16_g95, desaturateDot16_g95.xxx, clampResult15_g95 );
			float2 uv_DetailAlbedoMap = i.uv_texcoord * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
			float4 tex2DNode7_g94 = tex2D( _DetailAlbedoMap, ( uv_DetailAlbedoMap * TilingLayer2147 ) );
			...
			float clampResult15_g94 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g94 = (temp_cast_10 + (pow( tex2DNode7_g94 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g94 = dot( desaturateInitialColor16_g94, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g94 = lerp( desaturateInitialColor16_g94, desaturateDot16_g94.xxx, clampResult15_g94 );
			float4 switchResult175 = (((i.ASEVFace>0)?(( _Color * float4( desaturateVar16_g95 , 0.0 ) )):(( _Color2 * float4( desaturateVar16_g94 , 0.0 ) ))));
			float4 Albedomap34 = switchResult175;
			...
			float4 lerpResult182 = lerp( temp_cast_18 , temp_output_132_0 , _DoubleSidedBlend);
			float4 switchResult181 = (((i.ASEVFace>0)?(temp_output_132_0):(lerpResult182)));
			float temp_output_8_0_g127 = Albedomap34.r;
			float4 temp_output_5_0_g127 = ( temp_output_8_0_g127 * UNITY_LIGHTMODEL_AMBIENT );
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float fresnelNdotV11_g129 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode11_g129 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV11_g129, 5.0 ) );
			float4 Emissivemap31 = ( switchResult180 + ( lerp(temp_cast_15,switchResult181,_EnableReflection) + ( temp_output_5_0_g127 * _AmbientLight1 ) + ( ( ( 0.95 * fresnelNode11_g129 ) + 0.05 ) * _FresnelStrength1 ) ) );
			o.Emission = Emissivemap31.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g135 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * TilingLayer1148 ) );
			float2 uv_SpecGlossMap2 = i.uv_texcoord * _SpecGlossMap2_ST.xy + _SpecGlossMap2_ST.zw;
			float4 tex2DNode3_g134 = tex2D( _SpecGlossMap2, ( uv_SpecGlossMap2 * TilingLayer1148 ) );
			float switchResult177 = (((i.ASEVFace>0)?(( tex2DNode3_g135.a * _Metallic )):(( tex2DNode3_g134.a * _Metallic2 ))));
			float Metallic37 = switchResult177;
			o.Metallic = Metallic37;
			float switchResult176 = (((i.ASEVFace>0)?(( tex2DNode3_g135.r * _Glossiness )):(( tex2DNode3_g134.r * _Glossiness2 ))));
			float Roughness38 = switchResult176;
			o.Smoothness = Roughness38;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g132 = tex2D( _OcclusionMap, ( uv_OcclusionMap * TilingLayer1148 ) ).r;
			float blendOpDest2_g132 = ( 1.0 - _AoIntensity1 );
			float2 uv_OcclusionMap2 = i.uv_texcoord * _OcclusionMap2_ST.xy + _OcclusionMap2_ST.zw;
			float blendOpSrc2_g133 = tex2D( _OcclusionMap2, ( uv_OcclusionMap2 * TilingLayer2147 ) ).r;
			float blendOpDest2_g133 = ( 1.0 - _AoIntensity2 );
			float switchResult179 = (((i.ASEVFace>0)?(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g132 ) * ( 1.0 - blendOpDest2_g132 ) ) ))):(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g133 ) * ( 1.0 - blendOpDest2_g133 ) ) )))));
