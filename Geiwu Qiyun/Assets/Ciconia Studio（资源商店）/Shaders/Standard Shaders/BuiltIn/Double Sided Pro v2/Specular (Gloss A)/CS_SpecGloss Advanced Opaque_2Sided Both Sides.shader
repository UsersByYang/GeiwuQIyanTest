Shader "Ciconia Studio/Standard (Double Sided) V2 Legacy/Specular Gloss/Advanced (Both Sides)/Opaque"
{
	Properties
	{
		...
		[Space(10)]_DoubleSidedBlend("Double Sided Blend", Range( 0 , 1)) = 0
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g85 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * TilingLayer1148 ) ) ) , _BumpScale);
			float2 uv_DetailNormalMap = i.uv_texcoord * _DetailNormalMap_ST.xy + _DetailNormalMap_ST.zw;
			float TilingLayer2147 = _GeneralTilingLayer2;
			float3 lerpResult5_g86 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _DetailNormalMap, ( uv_DetailNormalMap * TilingLayer2147 ) ) ) , _DetailNormalMapScale);
			float3 switchResult178 = (((i.ASEVFace>0)?(lerpResult5_g85):(lerpResult5_g86)));
			float3 Normalmap19 = switchResult178;
			o.Normal = Normalmap19;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g88 = tex2D( _MainTex, ( uv_MainTex * TilingLayer1148 ) );
			...
			float clampResult15_g88 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g88 = (temp_cast_3 + (pow( tex2DNode7_g88 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g88 = dot( desaturateInitialColor16_g88, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g88 = lerp( desaturateInitialColor16_g88, desaturateDot16_g88.xxx, clampResult15_g88 );
			float2 uv_DetailAlbedoMap = i.uv_texcoord * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
			float4 tex2DNode7_g87 = tex2D( _DetailAlbedoMap, ( uv_DetailAlbedoMap * TilingLayer2147 ) );
			...
			float clampResult15_g87 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g87 = (temp_cast_10 + (pow( tex2DNode7_g87 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g87 = dot( desaturateInitialColor16_g87, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g87 = lerp( desaturateInitialColor16_g87, desaturateDot16_g87.xxx, clampResult15_g87 );
			float4 switchResult171 = (((i.ASEVFace>0)?(( _Color * float4( desaturateVar16_g88 , 0.0 ) )):(( _Color2 * float4( desaturateVar16_g87 , 0.0 ) ))));
			float4 Albedomap34 = switchResult171;
			o.Albedo = Albedomap34.rgb;
			float2 uv_EmissionMap = i.uv_texcoord * _EmissionMap_ST.xy + _EmissionMap_ST.zw;
			float2 uv_EmissionMap2 = i.uv_texcoord * _EmissionMap2_ST.xy + _EmissionMap2_ST.zw;
			float4 switchResult180 = (((i.ASEVFace>0)?(( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * TilingLayer1148 ) ) * _EmissiveIntensity1 )):(( _EmissionColor2 * tex2D( _EmissionMap2, ( uv_EmissionMap2 * TilingLayer2147 ) ) * _EmissiveIntensity2 ))));
			...
			float4 lerpResult184 = lerp( temp_cast_18 , temp_output_132_0 , _DoubleSidedBlend);
			float4 switchResult181 = (((i.ASEVFace>0)?(temp_output_132_0):(lerpResult184)));
			float temp_output_8_0_g114 = Albedomap34.r;
			float4 temp_output_5_0_g114 = ( temp_output_8_0_g114 * UNITY_LIGHTMODEL_AMBIENT );
			float4 Emissivemap31 = ( switchResult180 + ( lerp(temp_cast_15,switchResult181,_EnableReflection) + ( temp_output_5_0_g114 * _AmbientLight1 ) ) );
			o.Emission = Emissivemap31.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g110 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * TilingLayer1148 ) );
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float fresnelNdotV11_g111 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode11_g111 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV11_g111, 5.0 ) );
			float2 uv_SpecGlossMap2 = i.uv_texcoord * _SpecGlossMap2_ST.xy + _SpecGlossMap2_ST.zw;
			float4 tex2DNode3_g116 = tex2D( _SpecGlossMap2, ( uv_SpecGlossMap2 * TilingLayer1148 ) );
			float4 switchResult174 = (((i.ASEVFace>0)?(( ( _SpecularColor * tex2DNode3_g110 * _SpecularIntensity1 ) + ( ( ( 0.95 * fresnelNode11_g111 ) + 0.05 ) * _FresnelStrength1 ) )):(( _SpecularColor2 * tex2DNode3_g116 * _SpecularIntensity2 ))));
			float4 Specularmap37 = switchResult174;
			o.Specular = Specularmap37.rgb;
			float switchResult176 = (((i.ASEVFace>0)?(( tex2DNode3_g110.a * _Glossiness )):(( tex2DNode3_g116.a * _Glossiness2 ))));
			float Glossiness38 = switchResult176;
			o.Smoothness = Glossiness38;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g115 = tex2D( _OcclusionMap, ( uv_OcclusionMap * TilingLayer1148 ) ).r;
			float blendOpDest2_g115 = ( 1.0 - _AoIntensity1 );
			float2 uv_OcclusionMap2 = i.uv_texcoord * _OcclusionMap2_ST.xy + _OcclusionMap2_ST.zw;
			float blendOpSrc2_g117 = tex2D( _OcclusionMap2, ( uv_OcclusionMap2 * TilingLayer2147 ) ).r;
			float blendOpDest2_g117 = ( 1.0 - _AoIntensity2 );
			float switchResult179 = (((i.ASEVFace>0)?(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g115 ) * ( 1.0 - blendOpDest2_g115 ) ) ))):(( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g117 ) * ( 1.0 - blendOpDest2_g117 ) ) )))));
			float Aomap41 = switchResult179;
			o.Occlusion = Aomap41;
			o.Alpha = 1;
		}
